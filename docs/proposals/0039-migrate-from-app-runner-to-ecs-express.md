# 0039 --- Migrate from AWS App Runner to ECS Express Mode

**Date:** 2026-04-25
**Status:** Draft
**Author:** Architect Agent
**Related ADRs:** None yet (will produce ADR on acceptance)
...
Amazon ECS Express mode (announced November 2025) is the natural successor: it provides
the same "push a container, get a URL" developer experience that made App Runner attractive,
while giving full access to the underlying ECS primitives (service auto-scaling, security
groups) when needed. The `terraform-aws-modules/terraform-aws-ecs` community module already
ships an `express-service` sub-module, making the Terraform migration straightforward.
